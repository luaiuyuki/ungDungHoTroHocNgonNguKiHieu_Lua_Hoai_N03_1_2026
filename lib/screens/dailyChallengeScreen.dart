...
class _DailyChallengeScreenState extends State<DailyChallengeScreen> {
  ...
  Future<void> _processFrame(CameraImage image) async {
    ...
    try {
      ...
      final landmarks = await _handLandmarkerService.processFrame(
        bytes,
        image.width,
        image.height,
      );
      if (landmarks != null && landmarks.length == 63) {
        ...
        for (int i = 0; i < 21; i++) {
          transformedLandmarks[i * 3] =
              1.0 - landmarks[i * 3 + 1]; // Invert X to fix camera mirroring
          transformedLandmarks[i * 3 + 1] = landmarks[i * 3]; // Y
          transformedLandmarks[i * 3 + 2] = landmarks[i * 3 + 2]; // Z
        }
        final result = await _signClassifierService.predict(
        ...
        if (result != null && mounted) {
          setState(() {
            ...
            _targetConfidence =
                result.$3[_challengeLetters[_currentIndex]] ?? 0.0;
          });
          if (result.$1 == _challengeLetters[_currentIndex] &&
              result.$2 > 0.7) {
            _onAnswer(true);
          }
        }
      ...
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'Score: $_score/${_challengeLetters.length}',
                style: TextStyle(
                  fontSize: 24,
                            ...
                            children: [
                              Column(
                                children: [
                                  ...
                                  Text(
                                    '$_score/${_challengeLetters.length}',
                                    style: TextStyle(
                                      fontSize: 24,
                                      fontWeight: FontWeight.bold,
                                      color: textColor,
                              ...
                              Container(
                                ...
                                child: Center(
                                  child: Text(
                                    _challengeLetters[_currentIndex],
                                    style: TextStyle(
                                      fontSize: 48,
                                      fontWeight: FontWeight.w900,
                              ...
                              Column(
                                children: [
                                  ...
                                  const SizedBox(height: 4),
                                  Text(
                                    '${(_targetConfidence * 100).toStringAsFixed(0)}%',
                                    style: TextStyle(
                                      fontSize: 24,
                                      fontWeight: FontWeight.bold,
                                      color: _targetConfidence > 0.7
                                          ? Colors.green
                                          : textColor,
                                    ),
                                  ),
